...
json_data = JSON.dump(journal_entries)

# Store the serialized data in a file
File.open('data/journal_entries.json', 'w') do |file|
  file.write(json_data)  
end

# Save the persisted data that comes out of the file
persisted_data = File.open('data/journal_entries.json', 'r') do |file|
  file.read
end

# Check if the data read from the file is equal to the original object
if JSON.load(persisted_data) == journal_entries
  puts 'success!'
end
